...
abslayer_resolve_cuda_host_cxx() {
  ...
  ABSLAYER_CUDA_HOST_CXX_SOURCE="$configured_name"
  ABSLAYER_CUDA_HOST_CXX_PATH="$resolved_path"
  ABSLAYER_CUDA_HOST_CXX_FAMILY="clang"
  ABSLAYER_CUDA_HOST_CXX_MAJOR="$clang_major"
  ABSLAYER_CUDA_HOST_CXX_VERSION_FINGERPRINT="$("$ABSLAYER_CUDA_HOST_CXX_PATH" --version 2>&1 | cksum | awk '{print $1 "-" $2}')"
  if [[ -z "$ABSLAYER_CUDA_HOST_CXX_VERSION_FINGERPRINT" ]]; then
    echo "Could not fingerprint CUDA host C++ compiler: $ABSLAYER_CUDA_HOST_CXX_PATH" >&2
    return 1
  fi
  ...
  export MLX_CUDA_HOST_CXX ABSLAYER_CUDA_HOST_CXX_SOURCE
  export ABSLAYER_CUDA_HOST_CXX_PATH ABSLAYER_CUDA_HOST_CXX_FAMILY
  export ABSLAYER_CUDA_HOST_CXX_MAJOR ABSLAYER_CUDA_HOST_CXX_VERSION_FINGERPRINT
}
