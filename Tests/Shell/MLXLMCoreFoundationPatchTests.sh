...
grep -Fq 'EXPECTED_REVISION="14414441fa44f45eee35a61e9fa0bab577cf9734"' \
  "$PREPARE_SCRIPT"
grep -Fq 'mlx-swift-lm-corefoundation-linux.patch' "$PREPARE_SCRIPT"
grep -Fq 'git -C "$CHECKOUT" apply --reverse --check "$COREFOUNDATION_PATCH"' \
  "$PREPARE_SCRIPT"
...
TEST_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/abslayer-corefoundation.XXXXXX")"
cleanup() {
  local status=$?
  trap - EXIT
  rm -rf "$TEST_ROOT"
...
echo "ABSlayer mlx-swift-lm CoreFoundation compatibility patch checks passed"
